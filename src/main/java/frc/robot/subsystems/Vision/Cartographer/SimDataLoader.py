...
def simulateData(num_steps):
    ...
    for step in range(num_steps):
        robot_pose = gtsam.Pose3(gtsam.Rot3.Rz(np.random.normal(0, 0.03) + step * 0.1), gtsam.Point3(step * 0.1, step * 0.1, 0.0))
        ...
        for tag_id in tags_to_observe:
            ...
            step_data[tag_id] = noisy_pose  # Use the robot key here
