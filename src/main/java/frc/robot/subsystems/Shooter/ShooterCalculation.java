...
public class ShooterCalculation { // Spherical Coords
  ...
  public void setTarget(boolean isTeleop, boolean isFeederShot) {
    if (isFeederShot) {
      ...
      // this.targetZ = 9.806/2 * (((target.getX() - robotX)/2) * ((target.getX() - robotX)/2) +
      // ((target.getY() - robotY)/2) * ((target.getY() - robotY)/2));
      ...
      this.targetZ = target.getZ() + Units.inchesToMeters(0); // - 12 * 0.0254;
    } else {
  ...
  private double equalityCost(double phi, double theta, double t) {
    double dX =
        targetX
            - (robotX
                + Math.cos(robotTheta) * turretDisplacement
                + Math.cos(phi) * pivotDisplacement
                + Math.cos(phi) * Math.cos(theta) * outputDisplacementX
                + Math.cos(phi) * Math.sin(theta) * outputDisplacementY);
    double dY =
        targetY
            - (robotY
                + Math.sin(robotTheta) * turretDisplacement
                + Math.sin(phi) * pivotDisplacement
                + Math.sin(phi) * Math.cos(theta) * outputDisplacementX
                + Math.sin(phi) * Math.sin(theta) * outputDisplacementY);
    double dZ =
        targetZ
            - (robotZ
                + Math.cos(theta) * outputDisplacementY
                + Math.sin(theta) * outputDisplacementX);
  ...
  private double[] gradient(double phi, double theta, double t) {
    double dPhi =
        (constraintFunction(phi + epsilon, theta, t) - constraintFunction(phi - epsilon, theta, t))
            / (2 * epsilon);
    double dTheta =
        (constraintFunction(phi, theta + epsilon, t) - constraintFunction(phi, theta - epsilon, t))
            / (2 * epsilon);
    double dT =
        (constraintFunction(phi, theta, t + epsilon) - constraintFunction(phi, theta, t - epsilon))
            / (2 * epsilon);
  ...
  public double[] solveShot() {
    ...
    // IMPROVED INITIAL GUESS FROM CREATING A VIRTUAL TARGET AND APPLYING
    // https://www.desmos.com/calculator/0k8k97jbwz
    ...
    double theta =
        Math.atan(
            (vMag * vMag
                    - Math.sqrt(
                        vMag * vMag * vMag * vMag
                            - g * (g * virtualD * virtualD + 2 * (targetZ - robotZ) * vMag * vMag)))
                / (g * virtualD));
    double a = g / 2;
    ...
    if (b * b - 4 * a * c < 0 && Math.abs(robotVX) < 0.000001 && Math.abs(robotVY) < 0.000001) {
      if (enablePrinting) {
        // System.out.println("Impossible Standstill Shot: Too Far and/or Too Slow. Check
        // Parameters");
  ...
  public double[] solveShot(double initialPhi, double initialTheta, double initialT) {
    ...
    if (Double.isNaN(robotX)
        || Double.isNaN(robotY)
        || Double.isNaN(robotZ)
        || Double.isNaN(robotVX)
        || Double.isNaN(robotVY)
        || Double.isNaN(robotTheta)) {
      if (enablePrinting) {
    ...
    if (enablePrinting) {
      // System.out.println("Init " + phi + " " + theta + " " + t + " " + " Objective: " +
    ...
    // return null;
    if (enablePrinting) {
      // System.out.println("Failure " + phi + " " + theta + " " + t + " " + " Objective: " +
      // constraintFunction(phi, theta, t));
    }
    // System.out.println("");
  ...
  public double[] solveAll(boolean teleop, boolean feedershot) {
    ...
    setState(
        originalRX + (epsilon_jacobian * robotVX),
        originalRY + (epsilon_jacobian * robotVY),
        robotZ,
        robotTheta,
        robotVX,
        robotVY,
    ...
    return new double[] {
      ...
      standard[1],
      standard[2],
      (plus[0] - standard[0]) / (epsilon_jacobian),
      (plus[1] - standard[1]) / (epsilon_jacobian)
    };
  }

  public double[] solveWarmStart(
      double initialPhi, double initialTheta, double initialT, boolean teleop, boolean feedershot) {
  ...
  public double[] simulateShot(double phi, double theta, double t) {
    ...
    double z =
        (robotZ + Math.cos(theta) * outputDisplacementY + Math.sin(theta) * outputDisplacementX)
