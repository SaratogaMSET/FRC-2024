...
public class ShooterIOReal implements ShooterIO {
  ...
  public ShooterIOReal() {
    ...
    cc_cfg.MagnetSensor.AbsoluteSensorRange =
        AbsoluteSensorRangeValue.Signed_PlusMinusHalf; // IDK IF THIS WORKS
    cc_cfg.MagnetSensor.SensorDirection =
        SensorDirectionValue
            .Clockwise_Positive; // TODO: iDK IF THIS WORKS. When turret moves down angle increases.
    cc_cfg.MagnetSensor.MagnetOffset =
        -0.124
            + (0.038 * 2); // TODO: FIND THIS VALUE. Highest Bound = 14 degrees. 0.038889 rotations.
  ...
  public void updateInputs(ShooterIOInputs inputs) {
    ...
    inputs.pivotRad =
        Units.rotationsToRadians(angleMotor.getPosition().getValueAsDouble()); // 2 * Math.PI *
    // (-encoder.getAbsolutePosition().getValueAsDouble() -
    // ShooterPivotConstants.kEncoderOffset);
    // angleMotor.setPosition(inputs.pivotRad / (2 * Math.PI) * ShooterPivotConstants.kMotorGearing,
    // 0);
    inputs.pivotRadPerSec =
        Units.rotationsToRadians(
            angleMotor
                .getVelocity()
                .getValueAsDouble()); // angleMotor.getVelocity().getValueAsDouble() * 2 * Math.PI /
    // ShooterPivotConstants.kMotorGearing;
