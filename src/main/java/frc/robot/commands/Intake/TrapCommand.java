...
public class TrapCommand extends Command{
    ...
    public TrapCommand(IntakeSubsystem intakeSubsystem, RollerSubsystem roller, ElevatorSubsystem elevator, double shoulderAngle, double wristAngle){
    ...
    public void execute(){
        if(elevatorSubsystem.getAverageExtension() > 0.90){
            ...
            if (Math.abs(Math.toDegrees(intakeSubsystem.shoulderGetRads()) - Math.toDegrees(shoulderAngle)) <= Intake.Shoulder.POSITION_ERROR_TOLERANCE) {
                ...
                if(Math.abs(Math.toDegrees(intakeSubsystem.wristGetRads()) - Math.toDegrees(wristAngle)) <= 10){
